...
export default class Create3 extends React.Component{
    constructor(){
        super();
        this.state = {
            fname: "",
    ...
    updateLinks(){
        ...
        if(username){
            firebase.database().ref("dt_users/"+username).update({links: this.state.links}).then(()=>{
    ...
    render(){
        return(
            ...
            <div className="personal">
                <center>
            <h3>Links to your Profiles</h3>
            <div className="fieldsparent">
                <div className="inputbunch" id="ib">
                <input type="url" onChange={(event)=>{this.setState({fname: event.target.value})}} placeholder="Link (github.com/person4)" className="password"/><br/><br/>
                <input type="text" onChange={(event)=>{this.setState({fname: event.target.value})}} placeholder="Link Title (Github)" className="password1"/><br/><br/>
                </div>
                </div>
                <br/>
                <button className="createaccount" onClick={(event)=>{this.saveLinks(event)}}>Projects > </button>
                <br/><br/>
                <br/>
                <small className="errortext">{this.state.error}</small><br/>
                
                <small><b>3</b> of 4</small>
                </center>
            </div>
